...
const appsBuild = gulp.series(
    ...
    apps,
    gulp.series(cordova_apps(true)),
    gulp.parallel(listPostBuildTasks(APPS_DIR)),
);
gulp.task("apps", appsBuild);

const debugAppsBuild = gulp.series(
    gulp.parallel(clean_debug, gulp.series(clean_dist, debugDistBuild)),
    debug,
    gulp.series(cordova_apps(false)),
...
function processPackage(done, gitRevision, isReleaseBuild) {
    ...
    function write_package_file() {
        ...
        return packageJson.pipe(source("package.json")).pipe(gulp.dest(DIST_DIR));
    }
...
function dist_src() {
    const distSources = ["./src/**/*", "!./src/css/dropdown-lists/LICENSE", "!./src/support/**", "!./src/**/*.less"];

    return gulp.src(distSources, { base: "src" }).pipe(gulp.src("yarn.lock")).pipe(gulp.dest(DIST_DIR));
}
...
function dist_yarn() {
    return gulp
        .src([`${DIST_DIR}package.json`, `${DIST_DIR}yarn.lock`])
        .pipe(gulp.dest(DIST_DIR))
        .pipe(
            yarn({
                production: true,
...
function dist_locale() {
    return gulp.src("./locales/**/*", { base: "locales" }).pipe(gulp.dest(`${DIST_DIR}locales`));
}

function dist_libraries() {
    return gulp.src("./libraries/**/*", { base: "." }).pipe(gulp.dest(`${DIST_DIR}js`));
}

function dist_resources() {
    return gulp.src(["./resources/**/*", "!./resources/osd/**/*.png"], { base: "." }).pipe(gulp.dest(DIST_DIR));
}
...
function release_deb(arch, appDirectory, done) {
    ...
    return gulp.src([path.join(appDirectory, metadata.name, arch, "*")]).pipe(
        deb({
...
function cordova_copy_www() {
    return gulp.src(`${DIST_DIR}**`, { base: DIST_DIR }).pipe(gulp.dest(`${CORDOVA_DIST_DIR}www/`));
}

function cordova_resources() {
    return gulp.src("assets/android/**").pipe(gulp.dest(`${CORDOVA_DIST_DIR}resources/android/`));
}
...
function cordova_include_www() {
    return gulp
        ...
        .pipe(
            replace("<!-- CORDOVA_INCLUDE cordova.js -->", '<script type="text/javascript" src="cordova.js"></script>'),
        )
...
function cordova_copy_src() {
    return gulp
        .src([
            ...
            `!${CORDOVA_DIR}build_template.json`,
        ])
...
function cordova_manifestjson() {
    return gulp.src(`${DIST_DIR}package.json`).pipe(rename("manifest.json")).pipe(gulp.dest(CORDOVA_DIST_DIR));
}
...
function cordova_rename_build_json() {
    return gulp.src(`${CORDOVA_DIR}build_template.json`).pipe(rename("build.json")).pipe(gulp.dest(CORDOVA_DIST_DIR));
}
...
function cordova_debug_build(done) {
    cordova
        ...
        .then(function () {
            ...
            console.log(
                `APK has been generated at ${CORDOVA_DIST_DIR}platforms/android/app/build/outputs/apk/release/app-release.apk`,
            );
        ...
        function build(done2) {
            return cordova
                ...
                .then(function () {
                    ...
                    console.log(
                        "AAB has been generated at dist_cordova/platforms/android/app/build/outputs/bundle/release/app.aab",
                    );
                    done2();
                });
        },
...
async function cordova_debug_release() {
    ...
    return gulp
        .src(`${CORDOVA_DIST_DIR}platforms/android/app/build/outputs/apk/debug/app-debug.apk`)
...
async function cordova_release() {
    ...
    return gulp
        .src(`${CORDOVA_DIST_DIR}platforms/android/app/build/outputs/bundle/release/app.aab`)
        .pipe(rename(filename))
        .pipe(gulp.dest(RELEASE_DIR));
}
