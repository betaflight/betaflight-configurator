...
function imageToCharacter(data) {
    ...
    for (let i = 0, I = data.length; i < I; i += 4) {
        ...
        line += this.constants.MCM_COLORMAP[colorKey] || this.constants.MCM_COLORMAP["default"];
        if (line.length === 8) {
            char.push(line);
            line = "";
        }
...
LogoManager.openImage = function () {
    return new Promise((resolveOpenImage, rejectOpenImage) => {
        ...
        FileSystem.pickOpenFile(
            i18n.getMessage("fileSystemPickerFiles", { typeof: this.acceptFileTypes[0].description.toUpperCase() }),
