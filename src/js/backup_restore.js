            ...
            chrome.fileSystem.getWritableEntry(chosenFileEntry, function (fileEntryWritable) {
                ...
                chrome.fileSystem.isWritableEntry(fileEntryWritable, function (isWritable) {
                    if (isWritable) {
                        ...
                        const serialized_config_object = JSON.stringify(configuration, null, '\t');
                        const blob = new Blob([serialized_config_object], {type: 'text/plain'}); // first parameter for Blob needs to be an array
...
function configuration_restore(callback) {
        ...
        chosenFileEntry.file(function (file) {
            ...
            reader.onloadend = function (e) {
                if ((e.total != 0 && e.total == e.loaded) || GUI.isCordova()) {
                    ...
                    // validate
                    if (typeof configuration.generatedBy && compareVersions(configuration.generatedBy, CONFIGURATOR.BACKUP_FILE_VERSION_MIN_SUPPORTED)) {
    ...
    function migrate(configuration) {
        ...
        if (compareVersions(migratedVersion, '0.63.0') && !compareVersions(configuration.apiVersion, '1.7.0')) {
            ...
            const ports = [];
            for (const portIndex in configuration.SERIAL_CONFIG.ports) {
                const oldPort = configuration.SERIAL_CONFIG.ports[portIndex];
        ...
        if (semver.lt(migratedVersion, '0.66.0')) {
            ...
            for (const profileIndex in configuration.profiles) {
                ...
                for (const index in configuration.profiles[profileIndex].ServoConfig) {
                    const servoConfig = profiles[profileIndex].ServoConfig;

                    servoConfig[index].angleAtMin = 45;
                    servoConfig[index].angleAtMax = 45;
                    servoConfig[index].reversedInputSources = 0;

                    // set the rate to 0 if an invalid value is detected.
                    if (servoConfig[index].rate < -100 || servoConfig[index].rate > 100) {
                        servoConfig[index].rate = 0;
                    }
                }
                ...
                function upload_mode_ranges() {
                    if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_41)) {
                        if (configuration.MODE_RANGES_EXTRA == undefined) {
                            ...
                            for (const modeIndex in FC.MODE_RANGES) {
