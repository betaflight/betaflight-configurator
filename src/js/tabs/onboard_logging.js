...
onboard_logging.initialize = function (callback) {
    ...
    function load_html() {
        $("#content").load("./tabs/onboard_logging.html", function () {
            ...
            if (
                (FC.SDCARD.supported && deviceSelect.val() == 2) ||
    ...
    function populateLoggingRates(loggingRatesSelect) {
        ...
        for (let i = 0; i < sampleRateNum; i++) {
            ...
            loggingRatesSelect.append(
                `<option value="${i}">1/${2 ** i} (${loggingFrequency}${loggingFrequencyUnit})</option>`,
            );
        }
    ...
    function update_bar_width(bar, value, total, label, valuesAreKilobytes) {
        if (value > 0) {
            ...
            $("div", bar).text(
                (label ? `${label} ` : "") +
                    (valuesAreKilobytes ? formatFilesizeKilobytes(value) : formatFilesizeBytes(value)),
            );
        } else {
            bar.css({
                display: "none",
    ...
    $('input[name="expertModeCheckbox"]').on("change", () =>
        $("a.regular-button.require-msc-supported.save-flash").toggle(isExpertModeEnabled()),
    );
    ...
    function update_html() {
        ...
        update_bar_width(
            ...
            FC.DATAFLASH.usedSize,
            FC.DATAFLASH.totalSize,
            i18n.getMessage("dataflashUsedSpace"),
            false,
        );
        update_bar_width(
            $(".tab-onboard_logging .dataflash-free"),
            FC.DATAFLASH.totalSize - FC.DATAFLASH.usedSize,
            FC.DATAFLASH.totalSize,
            i18n.getMessage("dataflashFreeSpace"),
            false,
        );

        update_bar_width(
            $(".tab-onboard_logging .sdcard-other"),
            FC.SDCARD.totalSizeKB - FC.SDCARD.freeSizeKB,
            FC.SDCARD.totalSizeKB,
            i18n.getMessage("dataflashUnavSpace"),
            true,
        );
        update_bar_width(
            $(".tab-onboard_logging .sdcard-free"),
            FC.SDCARD.freeSizeKB,
            FC.SDCARD.totalSizeKB,
            i18n.getMessage("dataflashLogsSpace"),
            true,
        );
