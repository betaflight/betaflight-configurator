...
TABS.receiver.initialize = function (callback) {
    ...
    function process_html() {
        ...
        // rssi
        const rssiChannelE = $('select[name="rssi_channel"]');
        rssiChannelE.append(`<option value="0">${i18n.getMessage("receiverRssiChannelDisabledOption")}</option>`);
        //1-4 reserved for Roll Pitch Yaw & Throttle, starting at 5
        for (let i = 5; i < FC.RC.active_channels + 1; i++) {
            rssiChannelE.append(`<option value="${i}">${i18n.getMessage("controlAxisAux" + (i-4))}</option>`);
        }
