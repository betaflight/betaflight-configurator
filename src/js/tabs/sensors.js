...
TABS.sensors.initialize = function (callback) {
    ...
    $('#content').load("./tabs/sensors.html", function load_html() {
        ...
        const loadCheckBoxes = $('.tab-sensors .info .checkboxes input');
        loadCheckBoxes.parent().show();
        
        if (FC.CONFIG.boardType == 0 || FC.CONFIG.boardType == 2) { 
            if (!have_sensor(FC.CONFIG.activeSensors, 'acc')) {
                loadCheckBoxes.eq(1).prop('disabled', true);
            }
            if (!have_sensor(FC.CONFIG.activeSensors, 'mag')) {
                loadCheckBoxes.eq(2).prop('disabled', true);
            }
            if (!(have_sensor(FC.CONFIG.activeSensors, 'baro') || (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_40) && have_sensor(FC.CONFIG.activeSensors, 'gps')))) {
                loadCheckBoxes.eq(3).prop('disabled', true);
            }
            if (!have_sensor(FC.CONFIG.activeSensors, 'sonar')) {
                loadCheckBoxes.eq(4).prop('disabled', true);
            }
        } else {
            for (let i = 0; i <= 4; i++) {
                loadCheckBoxes.eq(i).prop('disabled', true);
                loadCheckBoxes.eq(i).parent().hide();
            }
        }
        ...
        $('.tab-sensors .info .checkboxes input').change(function () {
            ...
            $('.tab-sensors .info .checkboxes input').each(function () {
                setCheckBoxes.push($(this).prop('checked'));
            });

            $('.tab-sensors .rate select:first').change();

            ConfigStorage.set({'graphs_enabled': setCheckBoxes});
        });
        ...
        const accel_data = initDataArray(3);
        const mag_data = initDataArray(3);
        const altitude_data = initDataArray(1);
        const sonar_data = initDataArray(1);
        const debug_data = [
            initDataArray(1),
            initDataArray(1),
            initDataArray(1),
        ...
        $('.tab-sensors .rate select, .tab-sensors .scale select').change(function () {
            ...
            $('.tab-sensors .info .checkboxes input').each(function () {
                plotCheckBoxes.push($(this).prop('checked'));
            });
            ...
            // data pulling timers
            if (plotCheckBoxes[0] || plotCheckBoxes[1] || plotCheckBoxes[2]) {
        ...
        ConfigStorage.get('sensor_settings', function (result) {
            ...
            ConfigStorage.get('graphs_enabled', function (resultGraphs) {
                if (resultGraphs.graphs_enabled) {
                    const getCheckBoxes = $('.tab-sensors .info .checkboxes input');
                    for (let i = 0; i < resultGraphs.graphs_enabled.length; i++) {
                        getCheckBoxes.eq(i).not(':disabled').prop('checked', resultGraphs.graphs_enabled[i]).change();
                    }
                } else {
