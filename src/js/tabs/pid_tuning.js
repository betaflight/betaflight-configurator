...
pid_tuning.initialize = function (callback) {
    ...
    function pid_and_rc_to_form() {
        ...
        if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
            ...
            $('.pid_tuning .ROLL input[name="f"]').val(
                FC.ADVANCED_TUNING.feedforwardRoll > 0 ? FC.ADVANCED_TUNING.feedforwardRoll : PID_DEFAULT[4],
            );
            $('.pid_tuning .PITCH input[name="f"]').val(
                FC.ADVANCED_TUNING.feedforwardPitch > 0 ? FC.ADVANCED_TUNING.feedforwardPitch : PID_DEFAULT[9],
            );
            $('.pid_tuning .YAW input[name="f"]').val(
                FC.ADVANCED_TUNING.feedforwardYaw > 0 ? FC.ADVANCED_TUNING.feedforwardYaw : PID_DEFAULT[14],
            );
        ...
        if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
            ...
            dMinSwitch.prop(
                "checked",
                FC.ADVANCED_TUNING.dMinRoll > 0 || FC.ADVANCED_TUNING.dMinPitch > 0 || FC.ADVANCED_TUNING.dMinYaw > 0,
            );
                    ...
                    if (checked) {
                        if (
                            ...
                            $('.pid_tuning input[name="dMinRoll"]').val(
                                Math.min(Math.round($('.pid_tuning .ROLL input[name="d"]').val() * 0.57), 100),
                            );
                            $('.pid_tuning input[name="dMinPitch"]').val(
                                Math.min(Math.round($('.pid_tuning .PITCH input[name="d"]').val() * 0.57), 100),
                            );
                            $('.pid_tuning input[name="dMinYaw"]').val(
                                Math.min(Math.round($('.pid_tuning .YAW input[name="d"]').val() * 0.57), 100),
                            );
                            if (semver.eq(FC.CONFIG.apiVersion, API_VERSION_1_43)) {
                                $('.pid_tuning input[name="dMinRoll"]').val(
                                    Math.min(Math.round($('.pid_tuning .ROLL input[name="d"]').val() * 0.65), 100),
                                );
                                $('.pid_tuning input[name="dMinPitch"]').val(
                                    Math.min(Math.round($('.pid_tuning .PITCH input[name="d"]').val() * 0.65), 100),
                                );
                                $('.pid_tuning input[name="dMinYaw"]').val(
                                    Math.min(Math.round($('.pid_tuning .YAW input[name="d"]').val() * 0.65), 100),
                                );
                            }
                        } else {
                            $('.pid_tuning input[name="dMinRoll"]').val(FC.ADVANCED_TUNING.dMinRoll);
        ...
        if (semver.lt(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
            ...
            dtermLowpassEnabled.change(function () {
                ...
                const type =
                    FC.FILTER_CONFIG.dterm_lowpass_hz > 0
                        ? FC.FILTER_CONFIG.dterm_lowpass_type
            ...
            dtermLowpass2Enabled.change(function () {
                ...
                const type =
                    FC.FILTER_CONFIG.dterm_lowpass2_hz > 0
                        ? FC.FILTER_CONFIG.dterm_lowpass2_type
        ...
        if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
            gyroLowpassEnabled
                .prop(
                    "checked",
                    FC.FILTER_CONFIG.gyro_lowpass_hz !== 0 || FC.FILTER_CONFIG.gyro_lowpass_dyn_min_hz !== 0,
                )
                .change();
            dtermLowpassEnabled
                .prop(
                    "checked",
                    FC.FILTER_CONFIG.dterm_lowpass_hz !== 0 || FC.FILTER_CONFIG.dterm_lowpass_dyn_min_hz !== 0,
                )
                .change();
        } else {
            gyroLowpassEnabled.prop("checked", FC.FILTER_CONFIG.gyro_lowpass_hz !== 0).change();
            gyroLowpassDynEnabled
                .prop(
                    "checked",
                    FC.FILTER_CONFIG.gyro_lowpass_dyn_min_hz !== 0 &&
                        FC.FILTER_CONFIG.gyro_lowpass_dyn_min_hz < FC.FILTER_CONFIG.gyro_lowpass_dyn_max_hz,
    ...
    function form_to_pid_and_rc() {
        ...
        if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
            ...
            FC.TUNING_SLIDERS.slider_gyro_filter_multiplier =
                Math.round(TuningSliders.sliderGyroFilterMultiplier * 20) * 5;
        }
    }
    ...
    function hideUnusedPids() {
        ...
        isVisibleBaroMagGps |= hideSensorPid(
            $("#pid_baro"),
            have_sensor(FC.CONFIG.activeSensors, "baro") || have_sensor(FC.CONFIG.activeSensors, "sonar"),
        );
    ...
    function process_html() {
            ...
            setTimeout(function () {
                ...
                if (event) {
                    ...
                    if (
                        ...
                        semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_43)
                    ) {
        ...
        if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_42)) {
            ...
            if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
                ...
                allPidTuningSliders = $(
                    "#sliderMasterMultiplierLegacy, #sliderPDRatio, #sliderPDGain, #sliderFeedforwardGainLegacy",
                );
        ...
        $("a.update").click(function () {
            ...
            MSP.promise(MSPCodes.MSP_SET_PID, mspHelper.crunch(MSPCodes.MSP_SET_PID))
                ...
                .then(() =>
                    semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)
                        ? MSP.promise(
...
pid_tuning.updatePIDColors = function (clear = false) {
    ...
    setTuningElementColor(
        $('.pid_tuning input[name="dMinRoll"]'),
        FC.ADVANCED_TUNING_ACTIVE.dMinRoll,
        FC.ADVANCED_TUNING.dMinRoll,
    );
    setTuningElementColor(
        $('.pid_tuning input[name="dMinPitch"]'),
        FC.ADVANCED_TUNING_ACTIVE.dMinPitch,
        FC.ADVANCED_TUNING.dMinPitch,
    );
    setTuningElementColor(
        $('.pid_tuning input[name="dMinYaw"]'),
        FC.ADVANCED_TUNING_ACTIVE.dMinYaw,
        FC.ADVANCED_TUNING.dMinYaw,
    );
    setTuningElementColor(
