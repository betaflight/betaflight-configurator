...
FONT.openFontFile = function () {
    return new Promise(function (resolve) {
        chrome.fileSystem.chooseEntry(
            { type: "openFile", accepts: [{ description: "MCM files", extensions: ["mcm"] }] },
            function (fileEntry) {
...
OSD.loadDisplayFields = function () {
    ...
    OSD.ALL_DISPLAY_FIELDS = {
        ...
        ALTITUDE: {
            ...
            draw_order: 160,
            positionable: true,
            variants: ["osdTextElementAltitudeVariant1Decimal", "osdTextElementAltitudeVariantNoDecimal"],
            preview(osdData) {
...
OSD.chooseFields = function () {
    ...
    if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_43)) {
        OSD.constants.WARNINGS = OSD.constants.WARNINGS.concat([F.OVER_CAP]);
    }
    if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_45)) {
...
OSD.msp = {
    ...
    decode(payload) {
        ...
        d.state.isMax7456FontDeviceDetected =
            bit_check(d.flags, 5) ||
            (d.state.haveMax7456FontDeviceConfigured && semver.lt(FC.CONFIG.apiVersion, API_VERSION_1_43));
        ...
        async function updateOsdView() {
            ...
            MSP.promise(MSPCodes.MSP_OSD_CONFIG).then((info) => {
                ...
                for (let i = 0; i < OSD.constants.VIDEO_TYPES.length; i++) {
                    ...
                    const videoFormatOptionText = i18n.getMessage(
                        `osdSetupVideoFormatOption${inflection.camelize(type.toLowerCase())}`,
                    );
                    const $checkbox = $("<label/>").append(
                        $(`<input name="video_system" type="radio"/>${videoFormatOptionText}</label>`)
            ...
            chrome.fileSystem.chooseEntry(
                {
                    ...
                    accepts: [{ description: "MCM files", extensions: ["mcm"] }],
                },
                function (fileEntry) {
                    if (checkChromeRuntimeError()) {
                        return;
