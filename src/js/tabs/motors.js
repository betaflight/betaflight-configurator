...
motors.initialize = async function (callback) {
    ...
    function update_model(mixer) {
        ...
        const isMotorReorderingAvailable =
        ...
        self.escProtocolIsDshot = EscProtocols.IsProtocolDshot(
            FC.CONFIG.apiVersion,
            FC.PID_ADVANCED_CONFIG.fast_pwm_protocol,
        );
    }
    ...
    function process_html() {
        ...
        if (
            semver.lt(FC.CONFIG.apiVersion, API_VERSION_1_42) ||
            !(FC.MOTOR_CONFIG.use_dshot_telemetry || FC.MOTOR_CONFIG.use_esc_sensor)
        ) {
        ...
        function disableHandler(e) {
            if (e.target !== e.currentTarget) {
                ...
                if (item in defaultConfiguration) {
                    ...
                    console.log(
                        `Unknown item ${item} found with type ${e.target.type}, please add to the defaultConfiguration object.`,
                    );
                    self.configHasChanged = true;
                }
            ...
            mixerList.forEach(function (mixerEntry, mixerIndex) {
                if (mixerEntry.pos === selectIndex) {
                    mixerListElement.append(
                        `<option value="${mixerIndex + 1}">${mixerEntry.name.toUpperCase()}</option>`,
                    );
                }
            });
        }
        ...
        $(".tab-motors .rate select, .tab-motors .scale select").change(function () {
            ...
            function update_gyro_graph() {
                const gyro = [FC.SENSOR_DATA.gyroscope[0], FC.SENSOR_DATA.gyroscope[1], FC.SENSOR_DATA.gyroscope[2]];
        ...
        if (self.escProtocolIsDshot) {
            ...
            neutral3d =
                FC.MOTOR_3D_CONFIG.neutral > 1575 || FC.MOTOR_3D_CONFIG.neutral < 1425
                    ? 1500
                    : FC.MOTOR_3D_CONFIG.neutral;
        }
        ...
        function updateVisibility() {
            ...
            if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
                $('input[name="digitalIdlePercent"]').prop(
                    "disabled",
                    protocolConfigured &&
                        digitalProtocol &&
                        FC.ADVANCED_TUNING.idleMinRpm &&
                        FC.MOTOR_CONFIG.use_dshot_telemetry,
                );
            }

            if (FC.ADVANCED_TUNING.idleMinRpm && FC.MOTOR_CONFIG.use_dshot_telemetry) {
                const dynamicIdle = FC.ADVANCED_TUNING.idleMinRpm * 100;
                $("span.digitalIdlePercentDisabled").text(
                    i18n.getMessage("configurationDigitalIdlePercentDisabled", { dynamicIdle }),
                );
            } else {
            ...
            $("div.checkboxDshotBidir").toggle(
                protocolConfigured && semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_42) && digitalProtocol,
            );
            $("div.motorPoles").toggle(
                protocolConfigured && rpmFeaturesVisible && semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_42),
            );
        ...
        const ignoreKeys = ["PageUp", "PageDown", "End", "Home", "ArrowUp", "ArrowDown"];
        ...
        if (motorsRunning) {
            ...
            if (masterValue) {
                $("div.sliders input.master").val(masterValue).trigger("input");
            }
        }
            ...
            for (let i = 0; i < motorValues.length; i++) {
                ...
                if (
                    ...
                    if (rpmMotorValue > 999999) {
                        rpmMotorValue = `${(rpmMotorValue / 1000000).toFixed(
                            5 - (rpmMotorValue / 1000000).toFixed(0).toString().length,
                        )}M`;
                    }
    ...
    function setup_motor_output_reordering_dialog(callbackFunction, zeroThrottleValue) {
        ...
        const motorOutputReorderComponent = new MotorOutputReorderComponent(
            $("#dialogMotorOutputReorderContent"),
            callbackFunction,
            mixerList[FC.MIXER_CONFIG.mixer - 1].name,
            zeroThrottleValue,
            idleThrottleValue,
        );
