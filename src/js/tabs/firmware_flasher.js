    ...
    function onDocumentLoad() {
        ...
        function loadTargetList(targets) {
            ...
            if (result.selected_board) {
                ...
                $('select[name="board"]')
                    .val(selected ? result.selected_board : 0)
                    .trigger("change");
            }
        }
                                ...
                                if (e.total !== 0 && e.total === e.loaded) {
                                    ...
                                    if (file.name.split(".").pop() === "hex") {
                                        ...
                                        parseHex(self.intel_hex, function (data) {
                                            ...
                                            if (self.parsed_hex) {
                                                tracking.setFirmwareData(
                                                    tracking.DATA.FIRMWARE_SIZE,
        ...
        $("a.load_remote_file").click(function (evt) {
            ...
            function processBuildStatus(response, statusResponse, retries) {
                if (statusResponse.status === "success") {
                    ...
                    self.releaseLoader.loadTargetHex(
                        response.url,
            ...
            function requestCloudBuild(targetDetail) {
                ...
                self.releaseLoader.requestBuild(
                    request,
                    (response) => {
        ...
        $("span.progressLabel").on("click", "a.save_firmware", function () {
            chrome.fileSystem.chooseEntry(
                {
                    type: "saveFile",
                    suggestedName: self.targetDetail.file,
                    accepts: [{ description: "HEX files", extensions: ["hex"] }],
                },
                function (fileEntry) {
                    if (checkChromeRuntimeError()) {
                        return;
                                ...
                                fileEntry.createWriter(
                                    function (writer) {
                                        ...
                                        writer.onwriteend = function () {
                                            ...
                                            tracking.sendEvent(
                                                tracking.EVENT_CATEGORIES.FLASHING,
                                                "SaveFirmware",
                                                path,
                                            );
                                        };
        ...
        $("input.flash_on_connect")
            .change(function () {
                const status = $(this).is(":checked");

                if (status) {
                    const catch_new_port = function () {
                        PortHandler.port_detected(
                            "flash_detected_device",
                            function (resultPort) {
