        ...
        if (self.SHOW_OLD_BATTERY_CONFIG) {
            if (semver.gte(FC.CONFIG.flightControllerVersion, "3.1.0")) {
                ...
                for (const [index, value] of batteryMeterTypes.entries()) {
                    batteryMeterType_e.append(`<option value="${index}">${value}'</option>`);
                }
            ...
            for (const [index, value] of currentMeterTypes.entries()) {
                currentMeterType_e.append(`<option value="${index}">${value}</option>`);
            }
