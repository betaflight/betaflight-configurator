    ...
    function process_html() {
        ...
        if (self.SHOW_OLD_BATTERY_CONFIG) {
            if (semver.gte(FC.CONFIG.flightControllerVersion, "3.1.0")) {
                ...
                for (const index in batteryMeterTypes) {
                    batteryMeterType_e.append(`<option value="${index}">${batteryMeterTypes[index]}</option>`);
                }
            ...
            for (const index in currentMeterTypes) {
                currentMeterType_e.append(`<option value="${index}">${currentMeterTypes[index]}</option>`);
            }
