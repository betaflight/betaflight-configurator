...
TABS.led_strip.initialize = function (callback, scrollPosition) {
    ...
    function process_html() {
        ...
        $('.mainGrid').selectable({
            ...
            stop: function() {
                ...
                if (uiSelectedLast) {
                    ...
                    // set checkbox values
                    TABS.led_strip.overlays.forEach(function(letter) {
                        const featureO = $('.checkbox').find(`input.function-${letter}`);

                        const newVal = ($(uiSelectedLast).is(functionTag + letter));

                        if (featureO.is(':checked') !== newVal) {
                            featureO.prop('checked', newVal);
                            featureO.change();
                        }
                    });
        ...
        $('.gPoint').each(function(){
            ...
            for (const [modeIndex] of led.functions.entries()) {
                $(this).addClass('function-' + led.functions[modeIndex]);
            }

            for (const [directionIndex] of led.directions.entries()) {
