...
TuningSliders.updateExpertModePidSlidersDisplay = function () {
    const dGain =
        FC.TUNING_SLIDERS.slider_d_gain < this.NON_EXPERT_SLIDER_MIN ||
        FC.TUNING_SLIDERS.slider_d_gain > this.NON_EXPERT_SLIDER_MAX;
    const piGain =
        FC.TUNING_SLIDERS.slider_pi_gain < this.NON_EXPERT_SLIDER_MIN ||
        FC.TUNING_SLIDERS.slider_pi_gain > this.NON_EXPERT_SLIDER_MAX;
    const ffGain =
        FC.TUNING_SLIDERS.slider_feedforward_gain < this.NON_EXPERT_SLIDER_MIN ||
        FC.TUNING_SLIDERS.slider_feedforward_gain > this.NON_EXPERT_SLIDER_MAX;
    ...
    $(".baseSliderFeedforwardGain").toggleClass(
        "disabledSliders",
        !this.sliderPidsMode || (ffGain && !this.expertMode),
    );
    ...
    $(".subtab-pid .expertSettingsDetectedNote").toggle(
        !this.sliderPidsMode || ((basic || advanced) && !this.expertMode),
    );
};

TuningSliders.updateExpertModeFilterSlidersDisplay = function () {
    const gyroOutsideExpertMode =
        (FC.TUNING_SLIDERS.slider_gyro_filter_multiplier < this.NON_EXPERT_SLIDER_MIN_GYRO ||
            FC.TUNING_SLIDERS.slider_gyro_filter_multiplier > this.NON_EXPERT_SLIDER_MAX_GYRO) &&
        !this.expertMode;
    const dtermOutsideExpertMode =
        (FC.TUNING_SLIDERS.slider_dterm_filter_multiplier < this.NON_EXPERT_SLIDER_MIN_DTERM ||
            FC.TUNING_SLIDERS.slider_dterm_filter_multiplier > this.NON_EXPERT_SLIDER_MAX_DTERM) &&
        !this.expertMode;
    ...
    const gyroLowPassDisabled =
        parseInt(gyroLowpassDynMinFrequency.val()) === 0 &&
        parseInt(gyroLowpassFrequency.val()) === 0 &&
        parseInt(gyroLowpass2Frequency.val()) === 0;
    const dtermLowPassDisabled =
        parseInt(dtermLowpassDynMinFrequency.val()) === 0 &&
        parseInt(dtermLowpassFrequency.val()) === 0 &&
        parseInt(dtermLowpass2Frequency.val()) === 0;
...
TuningSliders.setExpertMode = function (expertModeEnabled) {
    ...
    if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
        ...
        $(".subtab-filter .nonExpertModeSlidersNote").toggle(
            (!this.GyroSliderUnavailable || !this.DTermSliderUnavailable) && !this.expertMode,
        );
    } else {
        ...
        $(".subtab-filter .legacyNonExpertModeSlidersNote").toggle(
            (!this.GyroSliderUnavailable || !this.DTermSliderUnavailable) && !this.expertMode,
        );
    }
};
...
TuningSliders.initPidSlidersPosition = function () {
    if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
        ...
        if (this.dMinFeatureEnabled) {
            this.sliderPDGain =
                Math.round(
                    (FC.ADVANCED_TUNING.dMinRoll /
                        this.sliderPDRatio /
                        this.sliderMasterMultiplierLegacy /
                        this.PID_DEFAULT[3]) *
                        10,
                ) / 10;
        } else {
            this.sliderPDGain =
                Math.round(
                    (FC.PIDS_ACTIVE[0][0] /
                        this.sliderMasterMultiplierLegacy /
                        (this.PID_DEFAULT[2] * (1 / D_MIN_RATIO))) *
                        10,
                ) / 10;
        }

        this.sliderFeedforwardGainLegacy =
            Math.round(
                (FC.ADVANCED_TUNING.feedforwardRoll / this.sliderMasterMultiplierLegacy / this.PID_DEFAULT[4]) * 10,
            ) / 10;
        ...
        this.sliderGyroFilterMultiplier =
            Math.round(
                ((FC.FILTER_CONFIG.gyro_lowpass_dyn_min_hz +
                    ...
                    (this.FILTER_DEFAULT.gyro_lowpass_dyn_min_hz +
                        this.FILTER_DEFAULT.gyro_lowpass_dyn_max_hz +
                        this.FILTER_DEFAULT.gyro_lowpass2_hz)) *
                    100,
            ) / 100;
        ...
        this.sliderDTermFilterMultiplier =
            Math.round(
                ((FC.FILTER_CONFIG.dterm_lowpass_dyn_min_hz +
                    ...
                    (this.FILTER_DEFAULT.dterm_lowpass_dyn_min_hz +
                        this.FILTER_DEFAULT.dterm_lowpass_dyn_max_hz +
                        this.FILTER_DEFAULT.dterm_lowpass2_hz)) *
                    100,
            ) / 100;
...
TuningSliders.updateSlidersWarning = function (slidersUnavailable = false) {
    ...
    if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
        ...
        enableWarning =
            ...
            FC.PIDS[0][2] > WARNING_DMAX_GAIN ||
            FC.ADVANCED_TUNING.dMinRoll > WARNING_DMIN_GAIN;
    } else {
        enableWarning =
            FC.PIDS[1][0] > WARNING_P_GAIN ||
            FC.PIDS[1][1] > WARNING_I_GAIN ||
            FC.PIDS[1][2] > WARNING_DMAX_GAIN ||
            FC.ADVANCED_TUNING.dMinPitch > WARNING_DMIN_GAIN;
    }
...
TuningSliders.updateFilterSlidersWarning = function () {
    ...
    $(".subtab-filter .slidersWarning").toggle(
        this.sliderGyroFilterMultiplier >= WARNING_FILTER_GYRO_HIGH_GAIN ||
            ...
            this.sliderDTermFilterMultiplier <= WARNING_FILTER_DTERM_LOW_GAIN,
    );
};
...
TuningSliders.updatePidSlidersDisplay = function () {
    ...
    if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
        $("#pid_main .ROLL .pid_data input, #pid_main .PITCH .pid_data input").each((_, el) =>
            $(el).prop("disabled", this.sliderPidsMode > 0),
        );
            ...
            pidElements.each(function (indexInput) {
                ...
                if (indexPid < 3 && indexInput < 3) {
                    ...
                    if (indexPid < 3 && indexInput === 3 && !this.dMinFeatureEnabled) {
                        ...
                        (indexPid < 3 && indexInput === 3 && this.dMinFeatureEnabled) ||
...
TuningSliders.readSimplifiedPids = function (updateSlidersOnly = false) {
    ...
    MSP.promise(MSPCodes.MSP_CALCULATE_SIMPLIFIED_PID, mspHelper.crunch(MSPCodes.MSP_CALCULATE_SIMPLIFIED_PID)).then(
...
TuningSliders.readSimplifiedGyroFilters = function () {
    ...
    MSP.promise(MSPCodes.MSP_CALCULATE_SIMPLIFIED_GYRO, mspHelper.crunch(MSPCodes.MSP_CALCULATE_SIMPLIFIED_GYRO)).then(
        () => this.updateGyroFilterSliderDisplay(),
    );
};
...
TuningSliders.validateTuningSliders = function () {
    MSP.promise(MSPCodes.MSP_VALIDATE_SIMPLIFIED_TUNING).then(() => {
        ...
        FC.TUNING_SLIDERS.slider_pids_mode = FC.TUNING_SLIDERS.slider_pids_valid
            ? FC.TUNING_SLIDERS.slider_pids_mode
            : 0;
        FC.TUNING_SLIDERS.slider_gyro_filter = FC.TUNING_SLIDERS.slider_gyro_valid
            ? FC.TUNING_SLIDERS.slider_gyro_filter
            : 0;
        FC.TUNING_SLIDERS.slider_dterm_filter = FC.TUNING_SLIDERS.slider_dterm_valid
            ? FC.TUNING_SLIDERS.slider_dterm_filter
            : 0;
...
TuningSliders.legacyCalculatePids = function (updateSlidersOnly = false) {
    ...
    FC.ADVANCED_TUNING.feedforwardRoll = Math.min(
        Math.round(FC.ADVANCED_TUNING.feedforwardRoll * this.sliderMasterMultiplierLegacy),
        MAX_FEEDFORWARD_GAIN,
    );
    FC.ADVANCED_TUNING.feedforwardPitch = Math.min(
        Math.round(FC.ADVANCED_TUNING.feedforwardPitch * this.sliderMasterMultiplierLegacy),
        MAX_FEEDFORWARD_GAIN,
    );
    FC.ADVANCED_TUNING.feedforwardYaw = Math.min(
        Math.round(FC.ADVANCED_TUNING.feedforwardYaw * this.sliderMasterMultiplierLegacy),
        MAX_FEEDFORWARD_GAIN,
    );

    if (this.dMinFeatureEnabled) {
        FC.ADVANCED_TUNING.dMinRoll = Math.min(
            Math.round(FC.ADVANCED_TUNING.dMinRoll * this.sliderMasterMultiplierLegacy),
            MAX_DMIN_GAIN,
        );
        FC.ADVANCED_TUNING.dMinPitch = Math.min(
            Math.round(FC.ADVANCED_TUNING.dMinPitch * this.sliderMasterMultiplierLegacy),
            MAX_DMIN_GAIN,
        );
        FC.ADVANCED_TUNING.dMinYaw = Math.min(
            Math.round(FC.ADVANCED_TUNING.dMinYaw * this.sliderMasterMultiplierLegacy),
            MAX_DMIN_GAIN,
        );
    }
...
TuningSliders.calculateLegacyGyroFilters = function () {
    ...
    FC.FILTER_CONFIG.gyro_lowpass_dyn_min_hz = Math.round(
        this.FILTER_DEFAULT.gyro_lowpass_dyn_min_hz * this.sliderGyroFilterMultiplier,
    );
    FC.FILTER_CONFIG.gyro_lowpass_dyn_max_hz = Math.round(
        this.FILTER_DEFAULT.gyro_lowpass_dyn_max_hz * this.sliderGyroFilterMultiplier,
    );
    FC.FILTER_CONFIG.gyro_lowpass2_hz = Math.round(
        this.FILTER_DEFAULT.gyro_lowpass2_hz * this.sliderGyroFilterMultiplier,
    );
...
TuningSliders.calculateLegacyDTermFilters = function () {
    // calculate, set and display new values in forms based on slider position
    FC.FILTER_CONFIG.dterm_lowpass_dyn_min_hz = Math.round(
        this.FILTER_DEFAULT.dterm_lowpass_dyn_min_hz * this.sliderDTermFilterMultiplier,
    );
    FC.FILTER_CONFIG.dterm_lowpass_dyn_max_hz = Math.round(
        this.FILTER_DEFAULT.dterm_lowpass_dyn_max_hz * this.sliderDTermFilterMultiplier,
    );
    FC.FILTER_CONFIG.dterm_lowpass_hz = Math.round(
        this.FILTER_DEFAULT.dterm_lowpass_hz * this.sliderDTermFilterMultiplier,
    );
    FC.FILTER_CONFIG.dterm_lowpass2_hz = Math.round(
        this.FILTER_DEFAULT.dterm_lowpass2_hz * this.sliderDTermFilterMultiplier,
    );
...
TuningSliders.legacyUpdateFilterSlidersDisplay = function () {
    ...
    if (
        parseInt($('.pid_filter input[name="gyroLowpassDynMinFrequency"]').val()) !==
    ...
    if (
        parseInt($('.pid_filter input[name="dtermLowpassDynMinFrequency"]').val()) !==
        ...
        parseInt($('.pid_filter select[name="dtermLowpassDynType"]').val()) !==
            this.FILTER_DEFAULT.dterm_lowpass_type ||
