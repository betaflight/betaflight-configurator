...
Features.prototype.isEnabled = function (featureName) {
    ...
    for (const index in self._features) {
        if (self._features[index].name === featureName && bit_check(self._featureMask, self._features[index].bit)) {
            return true;
        }
    }
    return false;
...
Features.prototype.updateData = function (featureElement) {
    ...
    if (featureElement.attr('type') === 'checkbox') {
        ...
        if (selectedBit !== -1) {
            let selectedFeature;
            for (let i = 0; i < controlElements.lenght; i++) {
