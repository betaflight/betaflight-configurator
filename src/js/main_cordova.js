...
const WEBVIEW = {
    ...
    advices: {
        installGoogleAndroidWebview: function (callback) {
            $(WEBVIEW.htmlElements.webview_step_msg).html(
                i18n.getMessage("cordovaWebviewInstall", {
                    app: WEBVIEW.apps[WEBVIEW.appsId.googleWebview].name,
                }),
            );
            $("#webview_step_btn1")
                .text(i18n.getMessage("cordovaWebviewInstallBtn"))
                .attr("app_id", WEBVIEW.appsId.googleWebview);
            callback();
        },
        updateGoogleAndroidWebview: function (callback) {
            $(WEBVIEW.htmlElements.webview_step_msg).html(
                i18n.getMessage("cordovaWebviewUpdate", {
                    app: WEBVIEW.apps[WEBVIEW.appsId.googleWebview].name,
                }),
            );
            $(WEBVIEW.htmlElements.webview_step_btn1)
                .text(i18n.getMessage("cordovaWebviewUpdateBtn"))
                .attr("app_id", WEBVIEW.appsId.googleWebview);
            callback();
        },
        ...
        uninstallGoogleAndroidWebview: function (callback) {
            $(WEBVIEW.htmlElements.webview_step_msg).html(
                i18n.getMessage("cordovaWebviewUninstall", {
                    app: WEBVIEW.apps[WEBVIEW.appsId.googleWebview].name,
                }),
            );
            $(WEBVIEW.htmlElements.webview_step_btn1)
                .text(i18n.getMessage("cordovaWebviewUninstallBtn1"))
                .attr("app_id", WEBVIEW.appsId.googleWebview);
            $(WEBVIEW.htmlElements.webview_step_btn2)
                .text(i18n.getMessage("cordovaWebviewUninstallBtn2"))
                .attr("app_id", WEBVIEW.appsId.googleWebview)
                .show();
            callback();
        },
        selectWebview: function (id, callback) {
            ...
            $(WEBVIEW.htmlElements.webview_step_msg).html(
                i18n.getMessage("cordovaWebviewEnable", {
                    app: app,
                }),
            );
    ...
    getAdvice2: function (callback) {
        ...
        if (self.uptodateApps.length > 0) {
            ...
            if (
                self.apps[WEBVIEW.appsId.googleWebview].installed &&
                self.apps[WEBVIEW.appsId.googleWebview].enabled &&
                self.apps[WEBVIEW.appsId.chrome].installed &&
                self.apps[WEBVIEW.appsId.chrome].enabled
            ) {
                self.advices.uninstallGoogleAndroidWebview(callback);
            } else if (
                !(
                    self.apps[WEBVIEW.appsId.googleWebview].installed && self.apps[WEBVIEW.appsId.googleWebview].enabled
                ) &&
    ...
    tryToFindUsedApp: function (callback) {
        ...
        for (let i = 0; i < appsId.length; i++) {
            ...
            if (self.matchingVersion === 1 && self.apps[id].used === "could") {
                ...
                self.usedApp = id;
                $(`li[app_id='${id}']`).append(
                    ` (<span style="color: green">${i18n.getMessage("cordovaWebviewUsed")}</span>)`,
                );
            }
