    ...
    switch (step) {
        ...
        case 2:
            ...
            for (var i = 0; i < self.flash_layout.sectors.length; i++) {
                for (var j = 0; j < self.flash_layout.sectors[i].num_pages; j++) {
                    if (self.options.erase_chip) {
                        ...
                        // local erase
                        var page_start =
                            self.flash_layout.sectors[i].start_address + j * self.flash_layout.sectors[i].page_size;
                        var page_end = page_start + self.flash_layout.sectors[i].page_size - 1;
                        for (var k = 0; k < self.hex.data.length; k++) {
                            var starts_in_page =
                                self.hex.data[k].address >= page_start && self.hex.data[k].address <= page_end;
            ...
            var erase_page = function () {
                var page_addr =
                    erase_pages[page].page * self.flash_layout.sectors[erase_pages[page].sector].page_size +
        ...
        case 4:
            ...
            var write = function () {
                if (bytes_flashed < self.hex.data[flashing_block].bytes) {
                    var bytes_to_write =
                        bytes_flashed + self.transferSize <= self.hex.data[flashing_block].bytes
                            ? self.transferSize
                            : self.hex.data[flashing_block].bytes - bytes_flashed;

                    var data_to_flash = self.hex.data[flashing_block].data.slice(
                        bytes_flashed,
                                    ...
                                    self.controlTransfer("in", self.request.GETSTATUS, 0, 0, 6, 0, function (data) {
                                        if (data[4] === self.state.dfuDNLOAD_IDLE) {
                                            ...
                                            TABS.firmware_flasher.flashProgress(
                                                (bytes_flashed_total / (self.hex.bytes_total * 2)) * 100,
                                            );
        ...
        case 5:
            ...
            var read = function () {
                if (bytes_verified < self.hex.data[reading_block].bytes) {
                    var bytes_to_read =
                        bytes_verified + self.transferSize <= self.hex.data[reading_block].bytes
                            ? self.transferSize
