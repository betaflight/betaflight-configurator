...
STM32_protocol.prototype.connect = function (port, baud, hex, options, callback) {
    ...
    if (self.options.no_reboot) {
        ...
        self.msp_connector.connect(
            self.port,
            self.options.reboot_baud,
            onConnectHandler,
            onTimeoutHandler,
            onFailureHandler,
        );
...
STM32_protocol.prototype.verify_flash = function (firstArray, secondArray) {
    for (let i = 0; i < firstArray.length; i++) {
        if (firstArray[i] !== secondArray[i]) {
            console.log(
                `Verification failed on byte: ${i} expected: 0x${firstArray[i].toString(16)} received: 0x${secondArray[
                    i
                ].toString(16)}`,
            );
            return false;
                ...
                if (self.options.erase_chip) {
                    ...
                    self.send([self.command.extended_erase, 0xbb], 1, function (reply) {
                        if (self.verify_response(self.status.ACK, reply)) {
                            ...
                            buff.push(checksum);
                            console.log(
                                `Erasing. pages: 0x00 - 0x${erasePagesN.toString(16)}, checksum: 0x${checksum.toString(
                                    16,
                                )}`,
                            );
                ...
                if (bytes_flashed < self.hex.data[flashing_block].bytes) {
                    ...
                    self.send([self.command.write_memory, 0xce], 1, function (reply) {
                        ...
                        if (self.verify_response(self.status.ACK, reply)) {
                            ...
                            self.send(
                                [addressArray[0], addressArray[1], addressArray[2], addressArray[3], addressChecksum],
                                1,
                                function (_reply) {
                                    if (self.verify_response(self.status.ACK, _reply)) {
                                        ...
                                        TABS.firmware_flasher.flashProgress(
                                            Math.round((bytes_flashed_total / (self.hex.bytes_total * 2)) * 100),
                                        );
                                    }
                                },
                            );
        ...
        case 6:
            ...
            const reading = function () {
                if (bytesVerified < self.hex.data[readingBlock].bytes) {
                    const bytesToRead =
                        bytesVerified + 256 <= self.hex.data[readingBlock].bytes
                            ? 256
                            : self.hex.data[readingBlock].bytes - bytesVerified;
                            ...
                            self.send(
                                ...
                                function (_reply) {
                                    ...
                                    if (self.verify_response(self.status.ACK, _reply)) {
                                        ...
                                        TABS.firmware_flasher.flashProgress(
                                            Math.round(
                                                ((self.hex.bytes_total + bytesVerifiedTotal) /
                                                    (self.hex.bytes_total * 2)) *
                                                    100,
                                            ),
                                        );
                                    }
        ...
        case 7:
            ...
            self.send([self.command.go, 0xde], 1, function (reply) {
                ...
                if (self.verify_response(self.status.ACK, reply)) {
                    ...
                    self.send(
                        [address[0], address[1], address[2], address[3], addressChecksum],
                        1,
                        function (response) {
                            if (self.verify_response(self.status.ACK, response)) {
                                // disconnect
