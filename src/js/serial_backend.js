...
function onOpen(openInfo) {
    if (openInfo) {
        ...
        gui_log(
            i18n.getMessage(
                "serialPortOpened",
                serial.connectionType === "serial" ? [serial.connectionId] : [openInfo.socketId],
            ),
        );
        ...
        MSP.send_message(MSPCodes.MSP_API_VERSION, false, false, function () {
            ...
            if (semver.gte(FC.CONFIG.apiVersion, CONFIGURATOR.API_VERSION_ACCEPTED)) {
                MSP.send_message(MSPCodes.MSP_FC_VARIANT, false, false, function () {
                    ...
                    if (FC.CONFIG.flightControllerIdentifier === "BTFL") {
                        MSP.send_message(MSPCodes.MSP_FC_VERSION, false, false, function () {
                            tracking.setFlightControllerData(
                                tracking.DATA.FIRMWARE_VERSION,
                ...
                tracking.sendEvent(
                    tracking.EVENT_CATEGORIES.FLIGHT_CONTROLLER,
                    "ConnectionRefusedFirmwareVersion",
    ...
    MSP.send_message(MSPCodes.MSP_STATUS, false, false, function () {
        ...
        if (semver.gt(FC.CONFIG.apiVersion, CONFIGURATOR.API_VERSION_MAX_SUPPORTED)) {
            ...
            problemItemTemplate
                .clone()
                .html(
                    i18n.getMessage(`reportProblemsDialog${problemName}`, [
            ...
            tracking.sendEvent(
                tracking.EVENT_CATEGORIES.FLIGHT_CONTROLLER,
                PROBLEM_ANALYTICS_EVENT,
                `${problemName};${CONFIGURATOR.API_VERSION_MAX_SUPPORTED};${FC.CONFIG.apiVersion}`,
            );
        }

        needsProblemReportingDialog =
            checkReportProblem("MOTOR_PROTOCOL_DISABLED", problemDialogList) || needsProblemReportingDialog;

        if (have_sensor(FC.CONFIG.activeSensors, "acc")) {
            needsProblemReportingDialog =
                checkReportProblem("ACC_NEEDS_CALIBRATION", problemDialogList) || needsProblemReportingDialog;
        }
