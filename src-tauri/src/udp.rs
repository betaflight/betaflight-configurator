...
fn run_flash(app: &AppHandle, ip: &str, port: u16, blocks: Vec<Block>) -> Result<(), String> {
    ...
    // the FC reports its flash layout in HELLO: config sector bounds, flash size, max write chunk
    let hello = link.request(PF_HELLO, &[], PF_HELLO, Duration::from_millis(500), 60)?;
    if hello.len() < 22 {
        return Err("short HELLO reply from FC".to_string());
    }
    let config_lo = u32::from_le_bytes([hello[6], hello[7], hello[8], hello[9]]);
    let config_hi = u32::from_le_bytes([hello[10], hello[11], hello[12], hello[13]]);
    let flash_size = u32::from_le_bytes([hello[14], hello[15], hello[16], hello[17]]);
    let max_chunk = (u16::from_le_bytes([hello[18], hello[19]]) as usize).max(4);
    let flash_end = FC_BASE as u64 + flash_size as u64;
    if flash_size == 0 || config_lo < FC_BASE || config_lo > config_hi || config_hi as u64 > flash_end {
        return Err("implausible flash layout in HELLO reply".to_string());
    }

    let mut runs: Vec<(u32, Vec<u8>)> = Vec::new();
    let mut stream: Vec<u8> = Vec::new();
    let mut sorted: Vec<Block> = Vec::new();
    for b in blocks {
        if b.data.is_empty() {
            continue;
        }
        let start = b.address;
        let end = start as u64 + b.data.len() as u64;
        if end <= config_lo as u64 || start >= config_hi {
            sorted.push(b);
            continue;
        }
        // the FC preserves its config sector, clip it out of any overlapping block
        if start < config_lo {
            let n = (config_lo - start) as usize;
            sorted.push(Block { address: start, data: b.data[..n].to_vec() });
        }
        if end > config_hi as u64 {
            let n = (config_hi - start) as usize;
            sorted.push(Block { address: config_hi, data: b.data[n..].to_vec() });
        }
    }
    ...
    for b in &sorted {
        ...
        if b.address as u64 + b.data.len() as u64 > flash_end {
            return Err(format!("block at {:#010x} exceeds the FC's {}k flash", b.address, flash_size / 1024));
        }
