...
const VALUE_FLAGS = new Set(["repo", "dev-ref", "pr", "min-api", "out", "labels-out", "fields-out", "source-url"]);
 ...
 */
/*
 * Marks a version read from a working tree, so the fixture says plainly that it
 * describes a developer's local firmware rather than anything anyone can check
 * out.
 */
...
async function main() {
    ...
    if (args.check) {
        await assertUpToDate(outputs, [
            flagFor("repo"),
            devRefFlag,
            ...["source-url", "min-api", "out", "labels-out", "fields-out"].map(flagFor),
        ]);
        return;
    }
