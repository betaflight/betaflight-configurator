...
import {
    ...
    type ModeRangeExtra,
    type RangeEntry,
} from "../../src/js/utils/modeRanges";

const SLOT_COUNT = 20;

interface EntriesByMode {
    arm?: ModeEntry[];
    angle?: ModeEntry[];
    beeper?: ModeEntry[];
}
...
function buildModes(entriesByMode: EntriesByMode = {}): Mode[] {
    return [
...
function rangeEntry(auxChannelIndex: number, start: number, end: number, modeLogic = 0): RangeEntry {
    return { kind: "range", auxChannelIndex, modeLogic, sliderRange: [start, end] };
}

function linkEntry(linkedTo: number, modeLogic = 0): LinkEntry {
...
function rebuildModes(
    modes: Mode[],
    { modeRanges, modeRangesExtra }: { modeRanges: ModeRange[]; modeRangesExtra: ModeRangeExtra[] },
): Mode[] {
    const rebuilt: Mode[] = modes.map((mode) => ({ id: mode.id, entries: [] }));
...
describe("mode range dirty serialization", () => {
    ...
    it("still reports a real edit as dirty", () => {
        ...
        (modes[0].entries[0] as RangeEntry).sliderRange = [1500, 2100];
