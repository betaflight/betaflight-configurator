 ...
 *
 * Kept as a separate describe block (rather than folded into the full-
 * pipeline tests above) specifically because it tests a different depth of
 * the pipeline — isolated ESKF predict/update calls, no RTS/Gauss-Newton —
 * and that isolation is what lets it attribute the observability gap to the
 * predict/update math itself rather than to smoother/iteration interactions.
 */
...
describe('isolated forward-only ESKF — FOGM GPS bias observability', () => {
